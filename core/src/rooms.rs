...
pub struct RoomState {
    ...
    pub role: Role,
    pub peer: PeerId,
    /// Our outgoing sequence number for this room.
    pub my_seq: u64,
    ...
    pub fn handle(&mut self, from: PeerId, env: Envelope, is_contact: bool) -> Vec<RoomEvent> {
        ...
        match env {
            Envelope::Invite {
                ...
                // No key yet — a zero key placeholder is unusable for
                // sealing/opening until KeyDelivery arrives.
                self.rooms.insert(
                    room_id_hex.clone(),
                    RoomState {
            ...
            Envelope::Join {
                ...
                let admission = if is_contact || self.auto_approve {
                    Admission::Approved
                } else {
